...
export class UsersClientProtobuf implements UsersClient {
  ...
  GetBatchUsers(request: GetBatchUsersRequest): Promise<GetBatchUsersResponse> {
    ...
    return promise.then((data) =>
      GetBatchUsersResponse.fromBinary(data as Uint8Array)
    );
  }
}
...
export const UsersMethodList = [UsersMethod.GetUser, UsersMethod.GetBatchUsers];
  ...
  switch (method) {
    ...
    case "GetBatchUsers":
      return async (
        ...
        await events.onMatch(ctx);
        return handleUsersGetBatchUsersRequest(
          ctx,
          service,
          data,
          interceptors
...
function handleUsersGetBatchUsersRequest<T extends TwirpContext = TwirpContext>(
  ...
  switch (ctx.contentType) {
    ...
    case TwirpContentType.Protobuf:
      return handleUsersGetBatchUsersProtobuf<T>(
        ctx,
        service,
        data,
        interceptors
...
async function handleUsersGetBatchUsersJSON<
...
>(
  ...
  try {
    const body = JSON.parse(data.toString() || "{}");
    request = GetBatchUsersRequest.fromJson(body, {
...
async function handleUsersGetUserProtobuf<
  T extends TwirpContext = TwirpContext
>(
  ctx: T,
  service: UsersTwirp,
...
async function handleUsersGetBatchUsersProtobuf<
  T extends TwirpContext = TwirpContext
>(
  ctx: T,
  service: UsersTwirp,
